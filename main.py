...
def mounts_host_type_index(host_type):
    if request.method == 'GET':
        ...
        if host_type == 'hosts':
            try:
                return ok(Mounts().add_nas_share(
                    host=data['name'],
...
def host_index(host_type, name):
    if request.method == 'GET':
        try:
            return ok(Mounts().nfs_info[host_type][name])
        except KeyError:
            return not_found()
        except Exception as e:
        ...
        try:
            Mounts().delete_host_name(host_type=host_type, name=name)
...
def host_mount_index(host_type, name, uuid):
    if request.method == 'GET':
        try:
            return ok([x for x in Mounts().nfs_info[host_type][name] if x['uuid'] == uuid])
        except KeyError:
            return not_found()
        except Exception as e:
            return error(e.message)
        ...
        try:
            Mounts().delete_host_mount(host_type=host_type, name=name, uuid=uuid)
        ...
        try:
            if host_type == 'hostgroups':
                return ok(Mounts().update_nas_share(
                    uuid=uuid,
